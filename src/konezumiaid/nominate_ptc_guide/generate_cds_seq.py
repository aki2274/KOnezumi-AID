...
def generate_exon_seq(transcript_record: TranscriptRecord) -> str:
    ...
    exon_seq_list = [
        ...
        for exon_start, exon_end in zip(transcript_record.exon_start_list, transcript_record.exon_end_list)
    ]
...
def get_startcodon_exon_index(transcript_record: TranscriptRecord) -> int:
    ...
    exon_index = None
    for n, (start, end) in enumerate(zip(transcript_record.exon_start_list, transcript_record.exon_end_list)):
...
def generate_cdsseq(transcript_record: TranscriptRecord) -> str:
    ...
    stopcodon_distance_from_exonend = transcript_record.exon_end_list[stopcodon_index] - transcript_record.cdsEnd
    if (
        stopcodon_distance_from_exonend == 0 and stopcodon_index - transcript_record.exonCount + 1 == 0
    ):  # the case of not having 3'UTR
        seq_to_stopcodon = exon_seq
    else:  # the case of having 3'UTR
        if stopcodon_index - transcript_record.exonCount + 1 == 0:  # the case of stopcodon is in the last exon
    ...
    if startcodon_index == 0:  # the case of startcodon is in the first exon
        ...
        for s in range(startcodon_index):
            startcodon_distance_from_first_exonstart += (
                transcript_record.exon_end_list[s] - transcript_record.exon_start_list[s]
            )
