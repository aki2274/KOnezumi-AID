...
def transform_c_to_t_guideseq_to_position(orf_seq: str, guide_seq: list[str]) -> list[int]:
    ...
    for seq in guide_seq:
        for seq_position in re.finditer(seq, orf_seq):
            ...
            position = re.search(r"(CAA|CAG|CGA)", seq[1:]).start() + seq_position.start() + 1
...
def transform_g_to_a_guideseq_to_position(orf_seq: str, guide_seq: list[str]) -> list[int]:
    ...
    for seq in guide_seq:
        for seq_position in re.finditer(seq, orf_seq):
            ...
            if rev_seq[0:3] == "GGT" and rev_seq[3:6] == "GGT":
                ...
                editable_codon_T_positions.append(seq_position.start() + len(seq) - 3 - 3)
            else:
                correction_by_target_window = re.search("GGT", rev_seq).start()
                editable_codon_T_positions.append(seq_position.start() + len(seq) - 3 - correction_by_target_window)
